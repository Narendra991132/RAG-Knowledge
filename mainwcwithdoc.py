...
def main():
    ...
    st.write("Integrates **Amazon Bedrock**, **Aurora**, and **pgvector** for conversational insights from enterprise documents.")

    col1, col2 = st.columns([1, 1.2])

    with col1:
        st.markdown("<div class='subheader'>Knowledge Source</div>", unsafe_allow_html=True)
        options = ["PDF Documents", "S3 Bucket", "YouTube", "CSV File", "PowerPoint", "Word Document"]
        ...
        if selected_source == "PDF Documents":
            ...
            if st.button("Load from S3") and key_selected:
                loader = S3FileLoader("aurora-genai-2023", f"documentEmbeddings/{key_selected}")
                docs = loader.load()
                all_text = " ".join([d.page_content for d in docs])
