    ...
    with col1:
        ...
        if selected_source == "PDF Documents":
            ...
            if st.button("Process PDF Documents") and files:
                ...
                st.success(f"✅ PDF processing completed successfully in {process_time}s!")
            ...
            if st.button("Process PowerPoint Files") and files:
                with st.spinner("📊 Processing PowerPoint presentations... Please wait."):
                    start_time = time.time()
                    for f in files:
                        path = save_uploaded_file(f)
                        ...
                        os.remove(path)
                    process_time = round(time.time() - start_time, 2)
                st.success(f"✅ PowerPoint files processed successfully in {process_time}s!")
            ...
            if st.button("Process CSV File") and file:
                with st.spinner("📈 Processing CSV data... Please wait."):
                    ...
                    all_text = " ".join([d.page_content for d in docs])
                    process_time = round(time.time() - start_time, 2)
                st.success(f"✅ CSV data processed successfully in {process_time}s!")
        ...
        #         all_text = " ".join([d.page_content for d in docs])
            ...
            if st.button("Load from S3") and key_selected:
                with st.spinner("☁️ Loading documents from S3... Please wait."):
                    start_time = time.time()
                    loader = S3FileLoader("knowledge-warehouse", f"documentEmbeddings/{key_selected}")
                    docs = loader.load()
                    all_text = " ".join([d.page_content for d in docs])
                    process_time = round(time.time() - start_time, 2)
                st.success(f"✅ S3 document loaded successfully in {process_time}s!")

        if all_text:
            with st.spinner("⚙️ Creating text chunks and storing embeddings in Aurora..."):
                chunks = get_text_chunks(all_text)
                vectorstore = get_vectorstore(chunks)
                st.session_state.conversation = get_conversation_chain(vectorstore, selected_llm)
            st.success("🎉 All documents processed and vectorstore initialized successfully!")
